...
def trigger_workflow(workflow_file="ci.yml"):
    ...
    if response.status_code == 204:
        ...
        console.print(f"[red]❌ Failed to trigger workflow: {response.status_code} — {response.text}[/red]")
        return
    ...
    for attempt in range(10):
        ...
        if runs:
            ...
            console.print(f"[cyan]Status:[/cyan] {status} | [cyan]Conclusion:[/cyan] {conclusion or 'in progress'}")

            if status == "completed":
                if conclusion == "success":
                    console.print(f"[green]✅ Workflow completed successfully![/green]")
                else:
                    console.print(f"[red]❌ Workflow failed: {conclusion}[/red]")
